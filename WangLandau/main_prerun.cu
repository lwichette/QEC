...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < num_interactions; i++)
    {
        ...
        std::string path = "init_" + std::to_string(X) + "/" + boundary + "/prob_" + std::to_string(prob_interactions) + "/X_" + std::to_string(X) + "_Y_" + std::to_string(Y) + "/seed_" + std::to_string(seed + i) + "/error_class_" + logical_error_type;
