...
def rescale_results_for_concatenation(results_x, results_y, minimum_deviation_energies):
    if len(minimum_deviation_energies) != 0:
        for i, e_concat in enumerate(minimum_deviation_energies):
            ...
            e_concat_index_in_following_interval = np.where(
                results_x[i + 1] == e_concat
            )[0]

            if (
                e_concat_index_in_preceeding_interval.size == 0
                or e_concat_index_in_following_interval.size == 0
            ):
                raise ValueError(
                    f"e_concat {e_concat} not found in one of the intervals."
                )

            e_concat_index_in_preceeding_interval = (
                e_concat_index_in_preceeding_interval[0]
            )
            e_concat_index_in_following_interval = e_concat_index_in_following_interval[
                0
            ]

            shift_val = (
                results_y[i][e_concat_index_in_preceeding_interval]
                - results_y[i + 1][e_concat_index_in_following_interval]
            )
...
def plot_log_g(file_name, exact_results=[]):
    ...
    args_string = (
        file_name.split("results/periodic/")[1].split(".txt")[0].replace("/", "_")
    )
...
def plot_log_g_diff_to_theory(file_name, exact_results=[]):
    ...
    args_string_curves = (
        file_name.split("results/")[1].split(".txt")[0].replace("/", "_")
    )
    plt.savefig(f"logPlot_{args_string_curves}.png")

    unique_xy_pairs_ordered = list(
        OrderedDict.fromkeys(xy_pairs)
    )  # Preserves the order
...
def main():
    file_name = "../results/periodic/prob_0.100000/X_12_Y_12/seed_44/error_class_I/intervals_10_iterations_1000_overlap_0.250000_walkers_5_seed_run_80_alpha_0.800000_beta_0.0000000100.txt"
    exact_results = extract_theory_results_from_file("theory_results.txt", 12, 12)
